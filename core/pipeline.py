...
async def analyze_chat_logic(
    ...
    prompt = f"""당신은 대한민국의 디지털 금융 범죄 분석 전문가입니다.
      ...
      [중요 지침 - 정상 대화 오인 방지]
      1. 일상적인 안부, 인사, 날씨 이야기, 단순 약속 등 '금전적 요구'나 '투자 권유' 등이 없는 평범한 대화는 무조건 상태를 'NORMAL'로 분류하고, 위험도 점수를 10% 이하로 낮게 설정하세요.
      2. 명백한 사기 유도 패턴이 있을 때만 'SUSPICIOUS' 또는 'FRAUD'로 판별하세요.
